...
class SPC_events_num:
    ...
    def run_continuously(self):
        with source(channels=['SATES30-RIXS-CAM01:EVENT_I', 'SATES30-RIXS-CAM01:EVENT_J','SATES30-RIXS-CAM01:EVENT_NUM']) as s:
            while True:
                m = s.receive()
                ix = m.data.pulse_id
              
                col_i = m.data.data['SATES30-RIXS-CAM01:EVENT_I'].value
                if col_i is None:
                    continue
                
                row_i = m.data.data['SATES30-RIXS-CAM01:EVENT_J'].value
                if row_i is None:
                    continue
    ...
    def plot_animation(self,name='SPC online Image',animate=True):
        ...
        #    return
        #self.fig,self.axs = plt.subplots(1,1,sharex=True,num=name)
        # self.fig.clf()
...
class SPC_Image:
    def __init__(self,frames_num = 500, slope = 2.92413825e-03, c2 = 7.58500298e-07 , pb = 1, ROI=[[0,1800],[0,1800]], gauss_fit = "True", fit_start_pix= 680, fit_end_pix=760,  pix_to_Energy = 1000/20 ):
...
SPC_run = SPC_Image(frames_num = 500, pb =2,  fit_start_pix= 750, fit_end_pix=870)
