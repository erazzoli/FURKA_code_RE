...
class TtProcessor:
    def __init__(self,Nbg = 10, pix_min = 0,  pix_max = 2000, n_std_filter_I0=0, WL_delay_stage_pos = 26.116,  poly_coef = [-1.42139290e-14,  2.16712147e-10, -8.44448276e-07,  1.57075480e-03, 2.50742803e+01], edge_pos_num = 500  ):
          self.bg = deque([],Nbg)
          self.sig = deque([],1)
          self.pos = deque([],1)
          self.I0 = deque([],1000)
    ...
    def run_continuously(self):
        with source(channels=['SATES31-CAMS187-RIXS1:x_profile','SAT-CVME-TIFALL5:EvtSet', 'SATES30-LSCP10-FNS:CH0:VAL_GET' ]) as s:
            while True:
                ...
                WL_delay_stage_pos = 26.12
                #print(len(self.dp_corr))
                #print(dp_corr_avg)
                actual_pos= self.STAGEpv.get()

                if  (len(self.dp_corr)==self.edge_pos_num) and  (abs(dp_corr_avg - self.WL_delay_stage_pos) > 0.015) and (abs(dp_corr_avg - self.WL_delay_stage_pos) < .3) and (np.mean(self.I0)>10000) and ((self.counter>1000)):
                      print( dp_corr_avg )
