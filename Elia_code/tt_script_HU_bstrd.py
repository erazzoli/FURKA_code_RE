...
class TtProcessor:
    def __init__(self,Nbg = 10, pix_min = 0,  pix_max = 2000, fb = False,  n_std_filter_I0=0, WL_delay_stage_pos = 15.25,  poly_coef = [5.18106410e-13,  -2.26414336e-09, 2.33922581e-06,  3.36690217e-03, 9.52477775e+01], edge_pos_num = 500  ):
          self.fb = fb
          self.bg = deque([],Nbg)
          self.sig = deque([],1)
          self.pos = deque([],1)
    ...
    def run_continuously(self):
        bs_pids  = BS("pids")
        bs_prof  = BS("SATES31-CAMS187-RIXS1:x_profile")
        bs_codes = BS("SAT-CVME-TIFALL5:EvtSet")
        ...
        for _ in bsstream:
                ...
                if(self.fb == True):
                                ...
                                if    (self.I0[-1] >  I0_filter) and (self.I0[-1] >  10000):
                                      ...
                                      self.edge_pos_I0.append(bs_I0.value)
                                      #print(self.edge_pos[-1])

                                dp_corr_avg = np.mean(self.dp_corr)
                                WL_delay_stage_pos = 15.25
